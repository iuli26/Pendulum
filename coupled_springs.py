...
class Ball:
    ...
    def draw(self):
        pygame.draw.circle(background, center=(self.x, self.y), color="red", radius=5 * m)
...
ball1 = Ball(x_ball, y_ball, [0, 0], m)
ball2 = Ball(x_ball*2, y_ball, [0,0], m)

spring_left = Spring(0, y_ref, coils=number_of_coils, k=20, x_ech=ball1.x, y_ech=ball1.y)
spring_right = Spring(width, y_ref, coils=number_of_coils, k=20, x_ech=ball2.x, y_ech=ball2.y)
spring_mid = Spring_mid(ball1, ball2, coils=number_of_coils, k=20)
...
while True:
    ...
    #axa()
    spring_left.draw_spring(ball1.x, ball1.y)
    spring_right.draw_spring(ball2.x, ball2.y)
    ...
    if click == 0:
        ...
        F1, F2 = 0, 0
        for spring in [spring_mid, spring_left]:
            F1 += spring.force(ball2, ball1)
        for spring in [spring_mid, spring_right]:
